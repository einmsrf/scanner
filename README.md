...
| `--retry <n>` | 瞬时网络错误（超时/连接被切断）的重试次数（默认 0，对连接被拒/解析失败不重试） |
...
- 报告会给出**失败原因分布**（timeout / refused / dns / tls / eof …）。超时占多数通常意味着
  出口被丢包或防护设备限速，而不是目标不可达，可降低 `--concurrency` 或稍后重试
- 若目标返回"暂停访问/维护中"页面，报告会明确标注 **目标暂停服务，结果不完整**，
  避免把残缺结果误读成"目标没问题"
